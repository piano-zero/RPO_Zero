...
def genera_pdf_ricevuta(profile, client, receipt_data, filename, is_credit_note=False):
    ...
    # --- NUOVO: PULIZIA TESTI PER EVITARE CRASH DEI FONT ---
    def clean_text(val):
        if isinstance(val, str):
            # Sostituisce apici curvi, virgolette tipografiche e trattini lunghi con versioni standard
            return val.replace("’", "'").replace("‘", "'").replace("“", '"').replace("”", '"').replace("–", "-")
        return val

    # Convertiamo gli oggetti sqlite3.Row in veri dizionari usando dict()
    profile = {k: clean_text(v) for k, v in dict(profile).items()}
    client = {k: clean_text(v) for k, v in dict(client).items()}
    receipt_data = {k: clean_text(v) for k, v in dict(receipt_data).items()}
    # -------------------------------------------------------
